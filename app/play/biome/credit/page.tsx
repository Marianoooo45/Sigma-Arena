...
export default function CreditDungeon() {
  ...
  return (
    <div className="fixed inset-0 overflow-hidden" style={{ ["--topbar" as any]: `${TOPBAR}px` }}>
      ...
      {/* Arrival FX — “Ion warp” violet */}
      {arrival && (
        <div className="cr-arrival" style={{ ["--tint" as any]: arrival } as React.CSSProperties}>
      ...
      <style jsx global>{`
        ...
        .btn-cta{
          ...
          padding: 1rem 1.6rem; border-radius: 9999px; color:#160c22;
        ...
        .btn-cta:disabled{
          ...
          background:#211232; border-color: color-mix(in srgb, var(--gx-line) 70%, transparent);
        }
        @keyframes ctaPulse { 0%,100% { opacity:.8 } 50% { opacity:1 } }

        /* Arrival FX — ion warp */
        .cr-arrival{ position:fixed; inset:0; z-index:60; pointer-events:none; --tint:${THEME.glow}; }
        .cr-arrival .vignette{
        ...
        .cr-arrival .swirl{
          ...
          background:
            radial-gradient(closest-side, rgba(255,255,255,.14), transparent 60%),
            repeating-conic-gradient(from 0deg, color-mix(in srgb, var(--tint) 42%, #c4b5fd 0%) 0 10deg, transparent 10deg 20deg);
        ...
        .cr-arrival .streaks::before,
        .cr-arrival .streaks::after{
        ...
        .cr-arrival .streaks::after{ animation-delay:60ms; opacity:.65; }
...
function Ring({ children }: { children?: React.ReactNode }) {
  return (
    <div className="relative" style={{ width: "clamp(420px, 68vmin, 840px)", aspectRatio: "1 / 1" }}>
      ...
      >
        <defs>
          <radialGradient id="ringGlow_cr" cx="50%" cy="50%" r="50%">
            <stop offset="72%" stopColor="rgba(168,85,247,0)" />
            <stop offset="100%" stopColor="rgba(168,85,247,0.16)" />
          </radialGradient>
          <linearGradient id="ringStroke_cr" x1="0" x2="1" y1="0" y2="1">
            <stop offset="0%"  stopColor={THEME.edge} />
            <stop offset="100%" stopColor={THEME.glow} />
          </linearGradient>
        </defs>

        <circle cx="50" cy="50" r="47" fill="url(#ringGlow_cr)" />
        <circle
          cx="50" cy="50" r="43.5"
          fill="none" stroke="url(#ringStroke_cr)" strokeWidth="1.15" strokeLinecap="round"
  ...
  return (
    ...
    >
      <svg width="34" height="38" viewBox="0 0 40 44">
        <defs>
          <linearGradient id="csGrad_cr" x1="0" x2="1" y1="0" y2="1">
