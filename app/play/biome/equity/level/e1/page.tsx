// app/play/biome/equity/level/e1/page.tsx
"use client";
...
const THEME = {
  ...
  edge: "#ffd0d5",                 // liseré clair (rose pâle)
};
...
export default function EquityLevelE1() {
  ...
  useEffect(() => {
    setLoading(true);
    fetchQuestionSet("equity", "e1", true)
      .then((qs) => {
  ...
  function finishLevel() {
    ...
    if (pass) {
      try { Progress.markCleared("equity", "e1"); } catch {}
    }
    // Ping UI
    ...
    router.replace("/play/biome/equity");
  }

  const headerTitle = "EQUITY • LEVEL 1";
  ...
  return (
    <div className="fixed inset-0 overflow-hidden" style={{ ["--topbar" as any]: `${TOPBAR}px` }}>
      ...
      {/* Map */}
      <BackToMapButton variant="equity" href="/play/biome/equity" placement="top-left" size="md" topbarHeight={TOPBAR} />
      ...
      {/* Styles (FX conservés, palette rouge) */}
      <style jsx global>{`
        /* ---- ruby neon utilities ---- */
        .ruby-outline{
          position:absolute; inset:-1px; border-radius:22px; padding:1px;
          background: conic-gradient(from 0deg, #ff4858, ${THEME.edge}, #ff4858 40%, #b83642, #ff4858 75%, ${THEME.edge}, #ff4858);
        ...
        .ruby-glow { box-shadow: 0 16px 40px rgba(0,0,0,.45), 0 0 30px ${THEME.glowDim} }
        ...
        .btn-crimson:hover{ transform: translateY(-2px) scale(1.02); filter: brightness(1.03); }
        .btn-crimson:active{ transform: translateY(0) scale(.99); }
        .btn-crimson .shine{
        ...
        .btn-crimson:hover .shine{ animation: shine 1s ease-out both; }
        @keyframes shine{ from{ transform:translateX(-120%); opacity:.0 } to{ transform:translateX(120%); opacity:.9 } }
        .btn-crimson .ripple{ position:absolute; width:6px; height:6px; border-radius:9999px; background: rgba(255,255,255,.7); opacity:0; pointer-events:none; transform: translate(-50%,-50%) scale(1) }
        .btn-crimson.rippling .ripple{ animation: ripple 600ms ease-out forwards; }
        ...
        .is-correct{ animation: pulseRuby 1200ms ease-in-out 1 both; }
        @keyframes pulseRuby{
        ...
        /* Arrival FX (identiques, teinte = THEME.glow) */
...
function LoadingCard() {
  return (
    <div
      className="relative w-[min(880px,92%)] rounded-3xl p-6 backdrop-blur grid place-items-center ruby-glow"
      style={{ background: "rgba(24,16,8,.55)", border: "1px solid rgba(255,255,255,.08)" }}
    >
      <div className="ruby-outline" />
...
function ErrorCard({ message }: { message: string }) {
  return (
    <div
      className="relative w-[min(880px,92%)] rounded-3xl p-6 backdrop-blur ruby-glow"
...
function QuestionCard({
  ...
  return (
    <div
      className="relative w-[min(880px,92%)] rounded-3xl p-5 sm:p-6 md:p-7 backdrop-blur ruby-glow sparks"
    ...
    >
      ...
      <div className="flex items-center justify-between gap-3 mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-extrabold tracking-wide" style={{ color: theme.glow }}>LEVEL 1 • EQUITY</div>
            <div className="text-[13px] text-[var(--gx-muted)]">Price Action Forge</div>
          </div>
        </div>
        <div
          className="px-3 py-1.5 rounded-full text-xs font-bold"
          style={{ color: "#20080a", background: `linear-gradient(90deg, ${theme.glow}, ${THEME.edge})`, boxShadow: `0 0 18px ${theme.glowDim}` }}
        >
          {idx + 1} / {total || 1}
        </div>
      </div>
      ...
      {q.type === "open" && revealed && (
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          <button className={`btn-crimson ${rippling ? "rippling" : ""}`} onClick={markOpenCorrect}>
      ...
      <div className="mt-5 flex items-center justify-between">
        ...
        {!revealed ? (
          <button
            className={`btn-crimson ${rippling ? "rippling" : ""}`}
  ...
  return (
    <span className="relative grid place-items-center rounded-xl p-1" style={{ filter: `drop-shadow(0 0 10px ${THEME.glowDim})` }}>
      <svg width="30" height="34" viewBox="0 0 40 44">
        ...
        <path d="M20 2 L34 8 V20 C34 29 28 36 20 42 C12 36 6 29 6 20 V8 Z" fill="rgba(0,0,0,.55)" stroke="url(#csGrad_eq)" strokeWidth="2" />
...
function IdBadge({ id, active, glow }: { id: string; active?: boolean; glow: string }) {
  return (
    <span
      ...
      style={{
        color: active ? "#20080a" : "rgba(255,255,255,.85)",
